...
public class JedisUtils {
    ...
    public static long setObjectList(String key, List<Object> value, int cacheSeconds) {
        ...
        try {
            ...
            byte[] _key = getBytesKey(key);
            result = jedis.rpush(_key, (byte[][]) list.toArray());
            if (cacheSeconds != 0) {
                jedis.expire(key, cacheSeconds);
            }
...
//			logger.debug("getResource.", jedis);
     ...
     * @return
     */
    public static Long fetchIncrNum(String key, int startNum) {
        Long num = null;
        String value = null;
        Jedis jedis = null;
        try {
    ...
    public static Long ttl(String key) {
        ...
        try {
            jedis = getResource();
            num = jedis.ttl(key);
            logger.debug("ttl {} = {}", key, num);
        } catch (Exception e) {
            logger.warn("ttl {} = {}", key, num, e);
        } finally {
            returnResource(jedis);
        }
        return num;
