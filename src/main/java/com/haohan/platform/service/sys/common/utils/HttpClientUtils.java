...
public class HttpClientUtils {
     ...
     * @param filePath
     */
    public void download(final String url, final String filePath, Map<String, String> headMap, StringEntity entity, HttpClientDownLoadProgress progress) {
        downloadExcutorService.execute(new Runnable() {

            @Override
            public void run() {
                httpDownloadFilePost(url, filePath, progress, headMap, entity);
            }
        });
    }
     ...
     * @param filePath
     */
    private void httpDownloadFilePost(String url, String filePath,
                                      HttpClientDownLoadProgress progress, Map<String, String> headMap, StringEntity entity) {
        CloseableHttpClient httpclient = HttpClients.createDefault();
        try {
...
//        System.out.println("responseContent = " + responseContent);
    ...
    public String httpPost(String url, Map<String, String> paramsMap,
        ...
        try {
            HttpPost httpPost = new HttpPost(url);
            setPostHead(httpPost, headMap);
            setPostParams(httpPost, paramsMap, charset);
            CloseableHttpResponse response = httpclient.execute(httpPost);
            try {
                System.out.println(response.getStatusLine());
                HttpEntity entity = response.getEntity();
     ...
     * @throws Exception
     */
    private void setPostParams(HttpPost httpPost, Map<String, String> paramsMap, String charset)
            throws Exception {
        if (paramsMap != null && paramsMap.size() > 0) {
            List<NameValuePair> nvps = new ArrayList<NameValuePair>();
            Set<String> keySet = paramsMap.keySet();
