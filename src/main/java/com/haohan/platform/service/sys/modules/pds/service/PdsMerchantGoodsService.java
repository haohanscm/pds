package com.haohan.platform.service.sys.modules.pds.service;
...
public class PdsMerchantGoodsService implements IGoodsConstant {
    ...
    @Autowired
    @Lazy(true)
    private PdsPlatformGoodsPriceService platformGoodsPriceService;
     ...
     * @return
     */
    public ApiRespPage fetchGoodsList(Goods goods, int pageNo, int pageSize) {
...
//            goods.setGoodsGift(oldGoods.getGoodsGift());
        ...
        result.put("goods", JacksonUtils.toJson(goods));
        baseResp.success();
    ...
    public int goodsDelete(Goods goods) {
        ...
        // 批量删除
        List<Goods> list = goodsService.findByGoodsSns(goods.getGoodsSn());
        if(Collections3.isEmpty(list)){
            return 0;
        }
        for (Goods g : list) {
            goodsService.deleteGoodsInfoExt(g);
        }
        return list.size();
    }
    ...
    public boolean goodsStatusModify(Goods goods) {
        ...
        goods.setId(oldGoods.getId());
        goodsService.save(goods);
        // 平台商家商品 下架时 删除 当前时间 商品的周期报价
        if(goods.getIsMarketable()==0){
            PdsPlatformGoodsPrice platformGoodsPrice = new PdsPlatformGoodsPrice();
            platformGoodsPrice.setPmId(oldGoods.getMerchantId());
            platformGoodsPrice.setQueryDate(new Date());
            platformGoodsPrice.setGoodsId(oldGoods.getId());
            platformGoodsPriceService.deleteBatch(platformGoodsPrice);
        }
        return true;
    }
    ...
    public int categoryDelete(GoodsCategory goodsCategory) {
        ...
        //  批量删除  即速删除
        List<GoodsCategory> list = goodsCategoryService.getForIds(goodsCategory.getId());
        int size = list.size() * 4 / 3 + 1;
        size = (!Collections3.isEmpty(list) && size > 8) ? size : 8;
        Map<String, List<String>> deleteMap = new HashMap<>(size);
        for (GoodsCategory g : list) {
            // 即速应用 分类
            if (StringUtils.isNoneEmpty(g.getJisuappId(), g.getCategorySn())) {
                // 启用同步
                if (!StringUtils.equals(g.getUpdateJisu(), ICommonConstant.YesNoType.no.getCode())) {
                    if (deleteMap.containsKey(g.getJisuappId())) {
                        deleteMap.get(g.getJisuappId()).add(g.getCategorySn());
                    } else {
                        List<String> categoryList = new ArrayList<>();
                        categoryList.add(g.getCategorySn());
                        deleteMap.put(g.getJisuappId(), categoryList);
                    }
                }
            }
        }
